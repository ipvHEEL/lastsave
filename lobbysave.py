...
class MainWindow(QMainWindow):
    ...
    def setup_teacher_workspace(self):
        ...
        self.date_label = QLabel(f"Выбранная дата: {self.calendar.selectedDate().toString('yyyy-MM-dd')}")
        layout.addWidget(self.date_label)

        # Таблица занятий
        self.schedule_table = QTableWidget()
        self.schedule_table.setColumnCount(2)
        self.schedule_table.setHorizontalHeaderLabels(["Время", "Предмет"])
        self.schedule_table.setRowCount(6)  # Предположим, максимум 6 пар в день
        layout.addWidget(self.schedule_table)
        # Сохраняем текущую дату
        self.selected_date = self.calendar.selectedDate().toPyDate()  # ← Используем правильный метод для PyQt6

        # Устанавливаем начальное расписание
        self.update_schedule_for_day(self.selected_date)

        container.setLayout(layout)
        self.workspace.addTab(container, "Мои занятия")

    def on_calendar_date_selected(self, qdate):
        """Обработчик выбора даты в календаре"""
        # Преобразуем QDate в Python date
        try:
            # PyQt6: используем toPyDate()
            self.selected_date = qdate.toPyDate()
        except:
            # Если нет toPyDate(), можно использовать toString и datetime
            date_str = qdate.toString("yyyy-MM-dd")
            self.selected_date = datetime.strptime(date_str, "%Y-%m-%d").date()

        self.date_label.setText(f"Выбранная дата: {self.selected_date.strftime('%Y-%m-%d')}")
        self.update_schedule_for_day(self.selected_date)

    def update_schedule_for_day(self, selected_date):
        conn = DB_Settings.get_connection()
        cursor = conn.cursor()
        ...
        try:
            ...
            # Запрашиваем расписание преподавателя на выбранный день
            cursor.execute("""
            ...
            # Настраиваем таблицу
            self.schedule_table.setRowCount(6)  # Фиксируем 6 возможных пар

            for record in rows:
                subject, start_time, duration_hours = record

                # Безопасное преобразование времени
                if isinstance(start_time, timedelta):
                    hour = int(start_time.total_seconds() // 3600)
                ...
                time_str = f"{hour:02}:00 – {end_hour:02}:00"

                self.schedule_table.setItem(pair_number, 0, QTableWidgetItem(time_str))
                self.schedule_table.setItem(pair_number, 1, QTableWidgetItem(subject))
