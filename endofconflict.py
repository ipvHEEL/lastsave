...
class MainWindow(QMainWindow):
    ...
    def setup_admin_workspace(self):  #
        ...
        btn_refresh_conflicts = QPushButton("Обновить конфликты")
        btn_refresh_conflicts.clicked.connect(self.load_conflicts)
        grid.addWidget(btn_refresh_conflicts, 2, 1)

        tabs.addTab(dashboard_tab, "Дашборд")
        self.workspace.addWidget(tabs)

        # Автоматически загружаем при входе
        self.load_conflicts()
    ...
    def setup_teacher_workspace(self):
        ...
        # В __init__ или setup_teacher_workspace
        self.lessons_table.cellClicked.connect(self.upd)
        # Кнопка запроса
    ...
    def on_lesson_selected(self):
        ...
        if not selected_items:
            QMessageBox.warning(self, "Ошибка", "Выберите занятие из таблицы")
            return None

        selected_row = selected_items[0].row()

        # Получаем данные из таблицы
        date_item = self.lessons_table.item(selected_row, 0)
        subject_item = self.lessons_table.item(selected_row, 1)
        classroom_item = self.lessons_table.item(selected_row, 2)  # предположим, это время начала
        duration_item = self.lessons_table.item(selected_row, 3)

        if None in [date_item, subject_item, classroom_item, duration_item]:
            QMessageBox.warning(self, "Ошибка", "Недостаточно данных в выбранной строке")
            return None

        date = date_item.text()
        subject = subject_item.text()
        start_time = classroom_item.text()  # временно используем classroom_item как время начала
        duration = duration_item.text().split()[0]  # получаем число часов
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                """
                cursor.execute(query, (subject, date, start_time, int(duration)))
                ...
                if result:
                    ...
                    QMessageBox.warning(self, "Ошибка", "Занятие не найдено в базе данных")
                    return None
        except Exception as e:
            QMessageBox.critical(self, "Ошибка", f"Ошибка при поиске в БД: {str(e)}")
            return None

    def upd(self):
        lesson_id = self.on_lesson_selected()
        if lesson_id is None:
            return

        # Открываем диалог для ввода новых данных
        dialog = QDialog(self)
        ...
        duration_combo.addItems(["1 ч", "2 ч", "3 ч", "4 ч"])

        layout.addRow("Новая дата:", date_edit)
        layout.addRow("Новое время:", time_edit)
        layout.addRow("Новая длительность:", duration_combo)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            new_start_time = time_edit.time().toString("HH:mm:ss")
            new_duration = int(duration_combo.currentText().split()[0])
            ...
            try:
                ...
                with conn.cursor() as cursor:
                    update_query = """
                        ...
                        WHERE id = %s;
                    """
                    cursor.execute(update_query, (new_date, new_start_time, new_duration, lesson_id))
    ...
    def load_conflicts(self):
        """Загружает список конфликтов в self.conflict_list"""
        try:
            conn = DB_Settings.get_connection()
            with conn.cursor() as cursor:
                # Получаем все записи из workload
                cursor.execute("SELECT index_code, group_name, teacher, total FROM workload")
                columns = [desc[0] for desc in cursor.description]
                results = [dict(zip(columns, row)) for row in cursor.fetchall()]

                # Предполагаем, что занятия начинаются с 8:00, каждые 2 часа, 5 дней в неделю
                schedule = []
                for record in results:
                    group = record['group_name']
                    teacher = record['teacher']
                    subject = record['index_code']
                    total_hours = int(record['total'])

                    # Рассчитываем количество пар (по 2 ч)
                    lessons_count = total_hours // 2
                    days = ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница']

                    # Распределяем занятия равномерно
                    for i in range(lessons_count):
                        day = days[i % len(days)]
                        pair_number = i % 4  # 4 пары в день
                        start_hour = 8 + pair_number * 2
                ...
                for i, entry1 in enumerate(schedule):
                    for j, entry2 in enumerate(schedule):
                        ...
                        if entry1["day"] == entry2["day"]:
                            if not (
                                    entry1["end_time"] <= entry2["start_time"] or
                                    entry1["start_time"] >= entry2["end_time"]
                            ):
                                # Конфликт по преподавателю
                                if entry1["teacher"] == entry2["teacher"]:
                                    conflict_text = f"Конфликт преподавателя\n{entry1['day']}\n{entry1['start_time']}–{entry1['end_time']}\n{entry1['subject']} vs {entry2['subject']}"
                                    conflicts.append(conflict_text)

                                # Конфликт по группе
                                if entry1["group"] == entry2["group"]:
                                    conflict_text = f"Конфликт группы\n{entry1['day']}\n{entry1['start_time']}–{entry1['end_time']}\n{entry1['subject']} vs {entry2['subject']}"
                                    conflicts.append(conflict_text)

                                # Конфликт по аудитории — если бы была аудитория
                                # можно добавить аналогично

                # Отображаем конфликты в интерфейсе
                self.conflict_list.clear()
                if conflicts:
                    self.conflict_list.addItems(conflicts)
                else:
                    self.conflict_list.addItem("Конфликтов нет")

        except Exception as e:
            QMessageBox.critical(self, "Ошибка", f"Не удалось загрузить конфликты: {str(e)}")
